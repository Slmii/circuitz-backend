...
impl Canister {
	...
	pub async fn update_settings(&self, settings: CanisterSettings) -> Result<(), (RejectionCode, String)> {
		api::call::call(Principal::management_canister(), "update_settings", (
			UpdateSettingsInput {
				canister_id: self.0,
				settings,
			},
	...
	pub async fn install_code<T: ArgumentEncoder>(
		...
		api::call::call(Principal::management_canister(), "install_code", (
			InstallCodeInput {
				mode,
				canister_id: self.0,
				wasm_module,
	...
	pub async fn uninstall_code(&self) -> Result<(), (RejectionCode, String)> {
		api::call::call(Principal::management_canister(), "uninstall_code", self.as_canister_id_arg()).await
	}

	pub async fn start(&self) -> Result<(), (RejectionCode, String)> {
		api::call::call(Principal::management_canister(), "start_canister", self.as_canister_id_arg()).await
	}

	pub async fn stop(&self) -> Result<(), (RejectionCode, String)> {
		api::call::call(Principal::management_canister(), "stop_canister", self.as_canister_id_arg()).await
	}

	pub async fn status(&self) -> Result<CanisterStatus, (RejectionCode, String)> {
		api::call
			::call(Principal::management_canister(), "canister_status", self.as_canister_id_arg()).await
			.map(|r: (CanisterStatus,)| r.0)
	}

	pub async fn delete(&self) -> Result<(), (RejectionCode, String)> {
		api::call::call(Principal::management_canister(), "delete_canister", self.as_canister_id_arg()).await
	}

	pub async fn deposit_cycles(&self) -> Result<(), (RejectionCode, String)> {
		api::call::call(Principal::management_canister(), "deposit_cycles", self.as_canister_id_arg()).await
	}

	pub async fn raw_rand(&self) -> Result<Vec<u8>, (RejectionCode, String)> {
		api::call::call(Principal::management_canister(), "raw_rand", ()).await.map(|r: (Vec<u8>,)| r.0)
	}

	pub async fn provisional_top_up(&self, amount: u128) -> Result<(), (RejectionCode, String)> {
		api::call::call(Principal::management_canister(), "provisional_top_up_canister", (
